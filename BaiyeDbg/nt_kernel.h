...
private:
	...
	void get_RtlpxLookupFunctionTable();

	void get_RtlCaptureImageExceptionValues();

	void get_PspReferenceSystemDll();

	void get_MiSectionControlArea();
	void get_MiReferenceControlAreaFile();

	void get_ObFastDereferenceObject();
...
typedef NTSTATUS(*Fn_NtDebugActiveProcess)(
	IN HANDLE DebugeeProcessHandle,
	IN HANDLE DebugObjectHandle
	);

typedef NTSTATUS(*Fn_NtCreateDebugObject)(
...
typedef VOID(*Fn_DbgkMapViewOfSection)(
	IN PEPROCESS_BY Process,
	IN PVOID SectionObject,
	IN PVOID SectionBaseAddress
	);

typedef VOID(*Fn_DbgkUnMapViewOfSection)(
	IN PEPROCESS_BY Process,
...
typedef PIMAGE_RUNTIME_FUNCTION_ENTRY(*Fn_RtlpxLookupFunctionTable)(
	PVOID ControlPc,
	PINVERTED_FUNCTION_TABLE_ENTRY TableEntry
	);

typedef VOID(*Fn_RtlCaptureImageExceptionValues)(
	IN PVOID ImageBase,
	OUT PIMAGE_RUNTIME_FUNCTION_ENTRY *RuntimeFunctionTable,
...
typedef PSECTION(*Fn_PspReferenceSystemDll)(
	PEX_FAST_REF FastRef
	);

typedef PCONTROL_AREA(*Fn_MiSectionControlArea)(
	PSECTION Section
	);

typedef PFILE_OBJECT(*Fn_MiReferenceControlAreaFile)(
	PCONTROL_AREA ControlArea
	);

typedef PFILE_OBJECT(*Fn_ObFastDereferenceObject)(
	PEX_FAST_REF FastRef,
	PVOID Object
	);