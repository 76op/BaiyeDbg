...
void emulate_vmcall(vcpu* const cpu) {
  ...
  //if (code & dbvm::hypercall_mask == dbvm::hypercall_mask)
  //{
  //    // handle the dbvm's hypercall
  //    switch (code) {
  //        
  //    }
  //}
...
void handle_exception_or_nmi(vcpu* const cpu) {
    ...
    interrupt_info.flags = vmx_vmread(VMCS_VMEXIT_INTERRUPTION_INFORMATION);

    switch (interrupt_info.interruption_type)
    {
        ...
        {
            if (interrupt_info.vector == exception_vector::debug)
            {
