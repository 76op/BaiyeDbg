...
void nt_kernel::get_apis()
{
	...
	get_RtlpxLookupFunctionTable();
	get_RtlCaptureImageExceptionValues();

	get_PspReferenceSystemDll();

	get_MiSectionControlArea();
	get_MiReferenceControlAreaFile();

	get_ObFastDereferenceObject();
...
void nt_kernel::get_PsQuerySystemDllInfo()
{
	pattern_search ps{
		...
		0x48, 0x83, 0x78, 0xCC, 0xCC,                   // cmp     qword ptr [rax+28h], 0
		0x74, 0xCC,                                     // jz      short loc_1406B781C
...
void nt_kernel::get_PspReferenceSystemDll()
{
	pattern_search ps{
		0x48, 0x89, 0x5C, 0x24, 0xCC,           // mov [rsp+arg_8], rbx
		0x48, 0x89, 0x6C, 0x24, 0xCC,           // mov [rsp+arg_10], rbp
		...
		0x48, 0x83, 0xEC, 0xCC,                 // sub rsp, 20h
		0x48, 0x8B, 0xF1,                       // mov rsi, rcx
		0xE8, 0xCC, 0xCC, 0xCC, 0xCC,           // call ObFastReferenceObject
		0x45, 0x33, 0xF6,                       // xor r14d, r14d
		0x48, 0x8B, 0xD8,                       // mov rbx, rax
		0x48, 0x85, 0xC0,                       // test rax, rax
		0x0F, 0x84, 0xCC, 0xCC, 0xCC, 0xCC      // jz loc_1408110B2
	};
	std::vector<ptr_t> matched;
	ps.search(0xcc, ntos_base(), ntos_size(), matched, 0, 3);
	ptr_t matched_ptr = matched[0];

	apis.insert(std::pair("PspReferenceSystemDll", reinterpret_cast<void *>(matched_ptr)));
}

void nt_kernel::get_MiSectionControlArea()
{
	pattern_search ps{
		0x48, 0x8B, 0x51, 0xCC,                 // mov     rdx, [rcx+28h]
		0x48, 0x8B, 0xC2,                       // mov     rax, rdx
		0x48, 0x83, 0xE0, 0xCC,                 // and     rax, 0FFFFFFFFFFFFFFFCh
		0xF6, 0xC2, 0xCC,                       // test    dl, 1
		0x0F, 0x85, 0xCC, 0xCC, 0xCC, 0xCC,     // jnz     loc_140429068
		0xF6, 0xC2, 0xCC,                       // test    dl, 2
		0x75, 0xCC,                             // jnz     short loc_14024891B
		0xC3                                    // retn
	};
	std::vector<ptr_t> matched;
	ps.search(0xcc, ntos_base(), ntos_size(), matched, 0, 3);
	...
	apis.insert(std::pair("MiSectionControlArea", reinterpret_cast<void *>(matched_ptr)));
}

void nt_kernel::get_MiReferenceControlAreaFile()
{
	pattern_search ps{
		0x48, 0x89, 0x5C, 0x24, 0xCC,           // mov     [rsp+arg_8], rbx
		...
		0x48, 0x83, 0xEC, 0xCC,                 // sub     rsp, 30h
		0x48, 0x8B, 0xF1,                       // mov     rsi, rcx
		0x48, 0x8D, 0x79, 0xCC,                 // lea     rdi, [rcx+40h]
		0x0F, 0x0D, 0x0F,                       // prefetchw byte ptr [rdi]
		0x4C, 0x8B, 0x07,                       // mov     r8, [rdi]
		0x41, 0xF6, 0xC0, 0xCC,                 // test    r8b, 0Fh
		0x74, 0xCC,                             // jz      short loc_140229C44
		0x49, 0x8D, 0x50, 0xCC,                 // lea     rdx, [r8-1]
		0x49, 0x8B, 0xC0,                       // mov     rax, r8
		0xF0, 0x48, 0x0F, 0xB1, 0x17,           // lock cmpxchg [rdi], rdx
		0x0F, 0x85, 0xCC, 0xCC, 0xCC, 0xCC      // jnz     loc_140229CF9
	};
	std::vector<ptr_t> matched;
	ps.search(0xcc, ntos_base(), ntos_size(), matched, 0, 3);
	ptr_t matched_ptr = matched[0];

	apis.insert(std::pair("MiReferenceControlAreaFile", reinterpret_cast<void *>(matched_ptr)));
}

void nt_kernel::get_ObFastDereferenceObject()
{
	pattern_search ps{
		0x48, 0x83, 0xEC, 0xCC,             // sub     rsp, 28h
		0x4C, 0x8B, 0xCA,                   // mov     r9, rdx
		0x0F, 0x0D, 0x09,                   // prefetchw byte ptr [rcx]
		0x48, 0x8B, 0x01,                   // mov     rax, [rcx]
		0x4C, 0x8B, 0xC0,                   // mov     r8, rax
		0x4C, 0x33, 0xC2,                   // xor     r8, rdx
		0x49, 0x83, 0xF8, 0xCC,             // cmp     r8, 0Fh
		0x73, 0xCC,                         // jnb     short loc_14029915A
		0x4C, 0x8D, 0x40, 0xCC,             // lea     r8, [rax+1]
		0xF0, 0x4C, 0x0F, 0xB1, 0x01,       // lock cmpxchg [rcx], r8
		0x75, 0xCC,                         // jnz     short loc_140299164
		0x48, 0x83, 0xC4, 0xCC,             // add     rsp, 28h
		0xC3                                // retn
	};
	std::vector<ptr_t> matched;
	...
	apis.insert(std::pair("ObFastDereferenceObject", reinterpret_cast<void *>(matched_ptr)));
}
...
void nt_kernel::get_RtlCaptureImageExceptionValues()
{
	pattern_search ps{
		0x48, 0x89, 0x5C, 0x24, 0xCC,           // mov     [rsp+arg_0], rbx
		0x57,                                   // push    rdi
		...
		0x41, 0xB8, 0xCC, 0xCC, 0xCC, 0xCC,     // mov     r8d, 3
		0xB2, 0xCC,                             // mov     dl, 1
		0xE8, 0xCC, 0xCC, 0xCC, 0xCC,           // call    RtlImageDirectoryEntryToData
		0x48, 0x89, 0x03,                       // mov     [rbx], rax
		0x48, 0x85, 0xC0,                       // test    rax, rax
		0x74, 0xCC                              // jz      short loc_14036FF76
	};
	std::vector<ptr_t> matched;
	ps.search(0xcc, ntos_base(), ntos_size(), matched, 0, 3);
	ptr_t matched_ptr = matched[0];

	apis.insert(std::pair("RtlCaptureImageExceptionValues", reinterpret_cast<void *>(matched_ptr)));
}
...
void nt_kernel::get_RtlpxLookupFunctionTable()
{
	pattern_search ps{
		0x48, 0x89, 0x5C, 0x24, 0xCC,                   // mov     [rsp+arg_0], rbx
		0x55,                                           // push    rbp
		...
		0x48, 0x83, 0xEC, 0xCC,                         // sub     rsp, 20h
		0x45, 0x33, 0xE4,                               // xor     r12d, r12d
		0xC6, 0x44, 0x24, 0xCC, 0xCC,                   // mov     [rsp+58h+arg_8], 0
		0x4C, 0x89, 0x62, 0xCC,                         // mov     [rdx+8], r12
		0x4C, 0x8B, 0xFA,                               // mov     r15, rdx
		0x44, 0x89, 0x62, 0xCC,                         // mov     [rdx+10h], r12d
		0x48, 0x8B, 0xF9,                               // mov     rdi, rcx
		0x48, 0x8B, 0x35, 0xCC, 0xCC, 0xCC, 0xCC        // mov     rsi, cs:RtlpInvertedFunctionTable
	};
	std::vector<ptr_t> matched;
	ps.search(0xcc, ntos_base(), ntos_size(), matched, 0, 3);
	ptr_t matched_ptr = matched[0];

	apis.insert(std::pair("RtlpxLookupFunctionTable", reinterpret_cast<void *>(matched_ptr)));
}